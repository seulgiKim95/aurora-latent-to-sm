...
def load_lite_decoder(model_aurora):
    new_vars = AuroraDatasetLiteDecoder.new_vars
    P = model_aurora.decoder.patch_size
    E = model_aurora.encoder.embed_dim
    model = MLPDecoderLite(
        surf_vars_new = new_vars,
        patch_size = P, # output layer (P*P)
        embed_dim = 2 * E, # input layer
        hidden_dims = [E, E, int(E/2)], # hidden layer
        # (E, E/2, E/2) in paper, while (E, E, E/2) in github code
    )
    return model
