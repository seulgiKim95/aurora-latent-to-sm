...
"""Adapted from Aurora's `aurora/model/aurora.py` by Seulgi Kim (2025-12-02): the only change from the original is that `forward()` also returns the decoder latent (`latent_decoder`)."""
...
class Aurora(torch.nn.Module):
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        # Add for Aurora-Lite-Decoder
        latent_decoder = x
        latent_decoder = rearrange(
            latent_decoder,
            "B (C H W) D -> B (H W) C D",
            C=patch_res[0],
            H=patch_res[1],
