...
from train_hydr.decoder_lite import MLPDecoderLite


def rollout(modelAurora: Aurora, modelDecoder: MLPDecoderLite, batch: Batch, steps: int) -> Generator[Batch, None, None]:
    """Perform a roll-out to make long-term predictions.

    Args:
        model (:class:`aurora_lite.Aurora`): The modified model for lite-decoder to roll out.
    ...
    for _ in range(steps):
        ...
        pred_Decoder = modelDecoder.forward(latent_decoder, batch.metadata.lat, batch.metadata.lon)
