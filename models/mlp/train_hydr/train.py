...
class LossCriterion():
    def __init__(self, land_sea_mask=None):
        ...
        # loss weights (alpha)
        self.alpha = self.set_weights(default={'L1':1., 'SSIM':0.})
        for k in self.alpha.keys():
            if hasattr(train_hydr.config.loss.alpha, k):
                self.alpha[k]["L1"] = getattr(train_hydr.config.loss.alpha, k).L1
                self.alpha[k]["SSIM"] = getattr(train_hydr.config.loss.alpha, k).SSIM
        ...
        self.data_range = {}
#       self.data_range['evaporation'] = 256.
#       self.data_range['precipitation'] = 256.
        pass
    ...
    def loss_ssim(self, pred, target, key):
        if key in self.data_range:
            loss = 1. - ms_ssim(pred.float(), target.float(), data_range=self.data_range[key])
        else:
            loss = 0.
        return loss
    ...
    def calc_var_loss(self, pred: dict, target: dict, key) -> torch.Tensor:
        ...
        l1_loss = self.loss_l1(p, t, key) if self.alpha[key]['L1'] > 0 else 0.
        ssim_loss = self.loss_ssim(p, t, key) if self.alpha[key]['SSIM'] > 0 else 0.
        var_loss = self.alpha[key]['L1'] * l1_loss + self.alpha[key]['SSIM'] * ssim_loss
        return var_loss
...
def train_model(modelAurora, modelDecoder, dataloader, optimizer, criterion, device, epoch, scaler=None, scheduler=None, verbose=True):
    ...
    for i, (batch_obj, target) in enumerate(dataloader):
...
#           torch.autograd.detect_anomaly() # for debugging
            ...
            if scaler is not None: # float16, scaler
                with autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    pred_Decoder = modelDecoder(latent_decoder, batch_obj.metadata.lat, batch_obj.metadata.lon)
