...
class LossCriterion():
    def __init__(self, land_sea_mask, porosity=None):
        ...
        # loss weights (alpha)
        self.alpha = self.set_weights(default={'L1':1., 'porosity':1.})
        for k1 in self.alpha.keys():
            if hasattr(train_hydr.config.loss.alpha, k1):
                ...
                self.alpha[k1]["porosity"] = getattr(train_hydr.config.loss.alpha, k1).porosity
    ...
    def set_static_device(self, device):
        self.porosity = torch.Tensor(self.porosity.copy())
        self.porosity = normalise_surf_var(self.porosity, 'swvl1')
        self.porosity = self.porosity.to(device)

        val = normalise_surf_var(0, 'swvl1')
        self.min_swvl1 = torch.full_like(self.porosity, val).to(device)
        return
    ...
    def loss_porosity(self, pred, target, key):
        if key not in ('swvl1') or self.porosity is None:
            return 0.
        max_val = self.porosity.broadcast_to(pred.shape)
        loss_sum_max = F.l1_loss(
            self.land_weights[key] * pred,
            self.land_weights[key] * max_val,
            reduction='sum',
            weight=self.latitude_weights * (pred > max_val),
        )
        min_val = self.min_swvl1.broadcast_to(pred.shape)
        loss_sum_min = F.l1_loss(
            self.land_weights[key] * pred,
            self.land_weights[key] * min_val,
            reduction='sum',
            weight=self.latitude_weights * (pred < min_val),
        )
        loss = (loss_sum_max + loss_sum_min) / (self.land_weights[key].sum() + self.epsilon)
        return loss
    ...
    def calc_var_loss(self, pred: dict, target: dict, key) -> torch.Tensor:
        ...
        porosity_loss = self.loss_porosity(p, t, key)
        var_loss = (self.alpha[key]['L1'] * l1_loss + self.alpha[key]['porosity'] * porosity_loss) / sum(self.alpha[key].values())
        return var_loss
...
#           torch.autograd.detect_anomaly() # for debugging
            ...
            if scaler is not None: # float16, scaler
                with autocast(device_type='cuda', dtype=torch.float16):
                    ...
                    pred_Decoder = modelDecoder(input_dict, latent_decoder, batch_obj.metadata.lat, batch_obj.metadata.lon, lsm_mask)
                ...
                with autocast(device_type='cuda', dtype=torch.bfloat16):
                    ...
                    pred_Decoder = modelDecoder(input_dict_bf16, latent_decoder, batch_obj.metadata.lat, batch_obj.metadata.lon, lsm_mask)
                    loss_value = criterion.loss(pred_Decoder, target) # compute loss
                loss_value.backward()
